...
% Function to perform Learning Vector Quantization (LVQ) classification with cross-validation
function [Perfomance, PerfomanceTotal, PerfomanceT, PerfomanceTotalT] = LVQ(inputc, KFold, Label, typelvq, ...
    ...
    if isnan(learnr) || (learnr <= 0) || (learnr >= 1)
        msgbox('Please Enter 0 < learnr < 1', '', 'warn');
        return;
    end
    if isnan(Nonedim) || (Nonedim < 1)
        msgbox('Please Enter Number Neurons > 1', '', 'warn');
        return;
    end
    if isnan(epoc) || (epoc < 1)
    ...
    for i = 1:KFold
        ...
        %% Step 9: Dimensionality Reduction (PCA or LDA)
        if Reduct == 2
            if ReductT == 1
        ...
        %% Step 10: LVQ Model Training
        % Create and configure the LVQ network
    ...
    ylabel(p(1), {'Predict Classes in'; 'each Kfold (LVQ)'});
    xlabel(p(1), 'Sample');
    p(1).YTick = 1:nclass;
    ...
    ylabel(p(3), {'Predict Classes in'; 'each Kfold (LVQ)'});
    xlabel(p(3), 'Sample');
    p(3).YTick = 1:nclass;
    p(3).YLim = [1 nclass + b];
    title(til, 'Test');
