function [Perfomance,PerfomanceTotal,PerfomanceT,PerfomanceTotalT] = LVQ(inputc,KFold,Label,typelvq,...
...
if isnan(learnr)||(learnr<=0)||(learnr>=1);msgbox('Please Enter 0<learnr<1','','warn');return;end
if isnan(Nonedim)||(Nonedim<1);msgbox('Please Enter Number Neurons >1','','warn');return;end
if isnan(epoc)||(epoc<1);msgbox('Please Enter Epochs >1','','warn');return;end
if isnan(maxf)||(maxf<1);msgbox('Please Enter Max Fail (Overfiting) >1','','warn');return;end
...
for i = 1:KFold
    ...
    %% LVQ
    net = lvqnet(Nonedim,learnr,char(typelvq));
    net.trainParam.epochs = epoc;
    net.trainParam.max_fail = maxf;
...
til=legend(p(1),Kf,'Location','best');title(til,'Traning');p(1).XLim=[0 length(INtr{1})];
ylabel(p(1),{'Predict Classes in'; 'each Kfold (LVQ)'});xlabel(p(1),'Sample');p(1).YTick=1:nclass;
p(1).FontName='Times New Roman';p(1).FontWeight='bold';p(1).YLim=[1 nclass+b];
...
til=legend(p(3),Kf,'Location','best');ylabel(p(3),{'Predict Classes in'; 'each Kfold (LVQ)'});
