...
nTestData=nTrainData;U=cell(KFold,1);U1=U;Kf=cell(1,KFold+1);Kf{1}='Real';y=cell(1,nclass);
Kfol=cell(KFold,1);
...
for i = 1:KFold
    ...
    elseif Reduct==3
        ...
        if ind==0;return;elseif length(ind)<nfeature;nfeature=length(ind);end
    ...
    %% Naive Bayesian
    if strcmp(typeBay,'mn'); mi=min(inputc);
        if ~isinteger(mi); typeBay='normal';
            warndlg('When Distribution Names is mn, the input data must be nonnegative integers, select DistributionNames normal');end
    end
