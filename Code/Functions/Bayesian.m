...
function [Perfomance, PerfomanceTotal, PerfomanceT, PerfomanceTotalT] = Bayesian(inputc, KFold, Label, typeBay, ...
    reduct, nfeature, redfeat, ValF, Nfeat1, Nfeat2, Alpha, Bin, cm, CM, ax5, ax6, ax9, ax10)
% This function performs Bayesian classification using k-fold cross-validation.
...
U = cell(KFold, 1); U1 = U; Kf = cell(1, KFold + 1); Kf{1} = 'Real'; y = cell(1, nclass);
Kfol = cell(KFold, 1);
...
for i = 1:KFold
    ...
    if strcmp(typeBay, 'mn')
        mi = min(inputc);
        if ~isinteger(mi)
            % Warn the user if the data is not suitable for multinomial distribution
            typeBay = 'normal';
            warndlg('When Distribution Names is mn, the input data must be nonnegative integers, select DistributionNames normal');
        end
    end

    % Train the Naive Bayes classifier
    CMdl = fitcnb(TrainInputs, TrainTargets, 'DistributionNames', char(typeBay));

    % Evaluate the classifier on the training data
    for j = 1:nclass
        nTrainData(i, j) = length(find(Label(trIdx, :) == j));
    end
    labeltrain = predict(CMdl, TrainInputs); INtr{i} = labeltrain;
    ...
    label = predict(CMdl, TestInputs); INte{i} = label;
...
til = legend(p(1), Kf, 'Location', 'best'); p(1).YLim = [1 nclass + b]; title(til, 'Training');
ylabel(p(1), {'Predict Classes in'; 'each Kfold (Bayesian)'}); xlabel(p(1), 'Sample'); p(1).YTick = 1:nclass;
...
til = legend(p(3), Kf, 'Location', 'best'); ylabel(p(3), {'Predict Classes in'; 'each Kfold (Bayesian)'});
