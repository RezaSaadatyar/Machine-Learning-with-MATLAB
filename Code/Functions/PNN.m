...
if isnan(sigma)||(sigma<0);msgbox('Please Enter Sigma > 0','','warn');return;end
...
for i = 1:KFold
    ...
    %% PNN
    if val==1
        mu=mean(TrainInputs,2);sd=std(TrainInputs,0,2);
        wkk=(TrainInputs-repmat(mu,1,size(TrainInputs,2)))./repmat(sd,1,size(TrainInputs,2));
    else
        wkk=TrainInputs;
    end
    % Train
    ...
    for jj=1:size(TrainInputs,2)
        x=TrainInputs(:,jj);if val==1;xn=(x-mu)./sd;else;xn=x;end
        if Type==1;zkk=wkk'*xn;okk(:,jj)=exp((zkk-1)/sigma);else
            reptXn=repmat(xn,1,size(wkk,2));r=sqrt(sum((reptXn-wkk).^2));
            okk(:,jj) = (1/ sqrt(2*pi*sigma))*exp(-(r)/(2*sigma));
        end
    end
    if numel(find(okk==inf))>numel(okk)/2;msgbox('Please active Normalize Input','','warn');return;end
    L=unique(TrainTargets);class=numel(L);index=cell(1,class);S=zeros(class,size(TrainInputs,2));
    for ii=1:class;index{ii}=find(TrainTargets==L(ii));end
    for j=1:class;S(j,:)=sum(okk(index{j},:));end;[~,labeltrain]=max(S);INtr{i}=labeltrain;
    ...
    for jj=1:size(TestInputs,2)
        x=TestInputs(:,jj);if val==1;xn=(x-mu)./sd;else;xn=x;end
        if Type==1;zkk=wkk'*xn;okk(:,jj)=exp((zkk-1)/sigma);else
            reptXn=repmat(xn,1,size(wkk,2));r=sqrt(sum((reptXn-wkk).^2));
            okk(:,jj) = (1/ sqrt(2*pi*sigma))*exp(-(r)/(2*sigma));
        end
    ...
    if numel(find(okk==inf))>numel(okk)/2;msgbox('Please active Normalize Input','','warn');return;end
    S=zeros(class,size(TestInputs,2));
    for ii=1:class;index{ii}=find(TrainTargets==L(ii));end
    for j=1:class;S(j,:)=sum(okk(index{j},:));end;[~,label]= max(S);INte{i}=label;
...
ylabel(p(1),{'Predict Classes in'; 'each Kfold (PNN)'});xlabel(p(1),'Sample');p(1).YTick=1:nclass;
...
til=legend(p(3),Kf,'Location','best');ylabel(p(3),{'Predict Classes in'; 'each Kfold (PNN)'});
