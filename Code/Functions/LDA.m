...
% Function to perform Linear Discriminant Analysis (LDA) classification with cross-validation
function [Perfomance, PerfomanceTotal, PerfomanceT, PerfomanceTotalT] = LDA(inputc, KFold, Label, tlda, ...
    ...
    for i = 1:KFold
        ...
        %% Dimensionality Reduction (PCA or FDA)
        if Reduct == 2
            if ReductT == 1
        ...
        %% LDA Model Training
        % Train the LDA model
        Mdl = fitcdiscr(TrainInputs, TrainTargets, 'DiscrimType', char(tlda));
    ...
    title(til, 'Training');
    p(1).YLim = [1 nclass + b];
    ylabel(p(1), {'Predict Classes in'; 'each Kfold (LDA)'});
    xlabel(p(1), 'Sample');
    p(1).YTick = 1:nclass;
    ...
    ylabel(p(3), {'Predict Classes in'; 'each Kfold (LDA)'});
    xlabel(p(3), 'Sample');
    p(3).YTick = 1:nclass;
    p(3).YLim = [1 nclass + b];
    title(til, 'Test');
