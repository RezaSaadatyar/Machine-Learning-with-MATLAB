...
for i = 1:KFold
   ...
   %% RBF
    ...
    if Vcent==1;[~,C]=kmeans(TrainInputs',rbf);Centers=C';else
    ind=randperm(size(TrainInputs,2));Centers=TrainInputs(:,ind(1:rbf));end
    dis=zeros(rbf);
    for ii=1:rbf;ci=Centers(:,ii);for j=1:rbf;cj=Centers(:,j);dis(ii,j)=sqrt(sum((ci-cj).^2));end;end
    d=max(dis(:));sigma=d/sqrt(2*rbf);G=zeros(rbf+1,size(TrainInputs,2));
    for ii=1:size(TrainInputs,2);x= TrainInputs(:,ii);g=zeros(1,rbf);
        for j= 1:rbf;cj=Centers(:,j);r=sqrt(sum((x-cj).^2));g(j)=exp(-r/(2*sigma));end
        G(:,ii)=[-1,g];
    end
