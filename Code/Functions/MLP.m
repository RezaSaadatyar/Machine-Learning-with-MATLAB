...
nTestData=nTrainData;U=cell(KFold,1);U1=U;Kf=cell(1,KFold+1);Kf{1}='Real';y=cell(1,nclass);Kfol=cell(KFold,1);

if isnan(Numh)||(ceil(Numh)<=0)||(ceil(Numh)>3);msgbox('Please Enter   0 < Number hidden layers < 4','','warn');return;end
if isnan(epocm)||(epocm<1);msgbox('Please Enter Epochs >1','','warn');return;end
if isnan(maxfm)||(maxfm<1);msgbox('Please Enter Max Fail (Overfiting) >1','','warn');return;end
...
for i = 1:KFold
    ...
    TrainInputs=inputc(trIdx,:)';TrainTargets=ind2vec(Label(trIdx,:)');
    TestInputs=inputc(teIdx,:)';labelTargetTest=ind2vec(Label(teIdx,:)');
    %% dimension reduction PCA OR FDA
    ...
    %% MLP
    net = feedforwardnet(NN,char(tlfmlp));net.trainParam.epochs=epocm;net.trainParam.max_fail=maxfm;
    c=net.numLayers;net.layers{c}.transferFcn =char(tTFun);%net.layers{2}.transferFcn = 'tansig';
    net=train(net,TrainInputs,full(TrainTargets));
    % Train
    for j=1:nclass;nTrainData(i,j)=length(find(Label(trIdx,:)==j));end
    labeltrain=net(TrainInputs);INtr{i}=vec2ind(labeltrain);
    ...
    label=net(TestInputs);INte{i}=vec2ind(label);
...
ylabel(p(1),{'Predict Classes in'; 'each Kfold (MLP)'});xlabel(p(1),'Sample');
p(1).YTick=1:nclass;p(1).FontName='Times New Roman';p(1).FontWeight='bold';p(1).XLim=[0 length(INtr{1})];
...
til=legend(p(3),Kf,'Location','best');title(til,'Test');ylabel(p(3),{'Predict Classes in'; 'each Kfold (MLP)'});
xlabel(p(3),'Sample');p(3).YTick=1:nclass;p(3).YLim=[1 nclass+b];p(3).XLim=[0 length(INte{1})];
p(3).FontName='Times New Roman';p(3).FontWeight='bold';
