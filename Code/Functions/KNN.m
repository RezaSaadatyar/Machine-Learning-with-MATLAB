...
if isnan(NumNneigh)||(NumNneigh<1);msgbox('Please Enter Number Neighbors > 1','','warn');return;end
...
for i = 1:KFold
    ...
    if strcmp(typeKnn,'kdtree')
        if ~ (strcmp(typeKdist,'euclidean')||strcmp(typeKdist,'cityblock')||strcmp(typeKdist,'minkowski')||strcmp(typeKdist,'chebychev'))
...
ylabel(p(1),{'Predict Classes in'; 'each Kfold (KNN)'});xlabel(p(1),'Sample');p(1).YTick=1:nclass;
...
til=legend(p(3),Kf,'Location','best');ylabel(p(3),{'Predict Classes in'; 'each Kfold (KNN)'});
