...
if Vinputc==1;msgbox('Please Select Input in Block Classification & Clustring','','warn');return;end
if get(Lals,'value')==1;msgbox('Please Select Labels in Block Classification & Clustring','','warn');return;end
if VCC==1;msgbox('Select Claasification OR Clusstering','','warn');return;end
if isnan(KFold)||(KFold<1);msgbox('Please Enter Number KFold > 1 ','','warn');return;end
if isnan(nfeature)||(nfeature>size(inputc,2))||(nfeature<1);msgbox(['Please Enter Number features;  0 < Number features < ' num2str(size(inputc,2))],'','warn');return;end     

if size(Lab,1)<size(Lab,2);Lab=Lab';end
if size(Lab,2)==1;Label=zeros(size(Lab));LL=unique(Lab);for i=1:length(LL)
Label(Lab==LL(i))=i;end;else;Lab=vec2ind(Lab');Label=zeros(size(Lab));LL=unique(Lab);
for i=1:length(LL);Label(Lab==LL(i))=i;end;Lab=Lab';
end
if (length(LL)>=length(Lab)/2)||(length(LL)==1);msgbox('Please Select Labels Properly ','','warn');return;end
if size(inputc,1)~=size(Lab,1);Lab=Lab(1:length(inputc),:);end
...
if VCC==2
    ...
    set(p22,'Visible','Off');
%     set(Cluster,'Value',1,'string',{'Select Classification Type:';'MLP';'Baysian';'SVM';'KNN';'LVQ';'LDA';'RBF';'PNN';'ELM';'PCA';'DT'});
    sclasify=get(Cluster,'String');Vclasify=get(Cluster,'Value');typeNSC=sclasify(Vclasify);
    if Vclasify==1;msgbox('Select Classification Type','','warn');return;end
    ...
    if strcmp(typeNSC,'MLP')
        ...
        set(NNhmlp,'Visible','Of');set(NNhmlp1,'Visible','Off');set(NNhmlp2,'Visible','Off');set(p8,'Visible','Off');
        if Numh==1;set(NNhmlp,'Visible','On');NN=NNh;set(NNhmlp2,'Visible','Off');set(NNhmlp1,'Visible','Off');
            if isnan(NNh);msgbox('Please Enter Number Neurons Layer 1','','warn');return;end
        elseif Numh==2;set(NNhmlp1,'Visible','On');NN=[NNh NNh1];set(NNhmlp,'Visible','On');set(NNhmlp2,'Visible','Off');
            if isnan(NNh);msgbox('Please Enter Number Neurons Layer 1','','warn');return;end
            if isnan(NNh1);msgbox('Please Enter Number Neurons Layer 2','','warn');return;end
        elseif Numh==3;set(NNhmlp2,'Visible','On');NN=[NNh NNh1 NNh2];set(NNhmlp1,'Visible','On');set(NNhmlp,'Visible','On');
            if isnan(NNh);msgbox('Please Enter Number Neurons Layer 1','','warn');return;end
            if isnan(NNh1);msgbox('Please Enter Number Neurons Layer 2','','warn');return;end
            if isnan(NNh2);msgbox('Please Enter Number Neurons Layer 3','','warn');return;end
        end
...
elseif VCC==3
    ...
    if strcmp(typeNSC,'Kmeans')
        set(p21,'Visible','On');set(p22,'Visible','Off');%set(p1,'Visible','On');
        if isnan(Itkm);msgbox('Please Enter Number Iteration','','warn');return;end
        [idx,~] = kmeans(inputc,NumClass,'MaxIter',Itkm,'Distance',char(typKmdist));
        aa = (Label-idx);aa = aa(aa==0);accuracy = (length(aa)/length(idx))*100;disp('accuracy kmeans:');disp(accuracy)
    elseif strcmp(typeNSC,'FCM')
        set(p21,'Visible','On');set(p22,'Visible','Off');%set(p1,'Visible','Off');
    ...
    % elseif strcmp(typeNSC,'SOFM')
    %     set(p21,'Visible','Off');set(p22,'Visible','On');nRow=str2double(get(Nonedim1,'String'));
    %     nCol=str2double(get(Nonedim2,'String'));epoch=str2double(get(Epoch,'String'));eta0=str2double(get(Eta,'String'));
    %     tau=str2double(get(Tau,'String'));sig0 =str2double(get(Sig,'String'));
%         [Perfomance,PerfomanceTotal]=SFC(epoch,eta0,sig0,nRow,nCol,tau,Lab,inputc,KFold,cm,CM,ax5,ax6,ax9,ax10);
    end
end
end