...
for i = 1:KFold
   ...
   %% PCA
    ...
    labeltrain = pca_test(mdl,TrainInputs);INtr{i}=labeltrain;
    ...
    label = pca_test(mdl,TestInputs);INte{i}=label;
...
ylabel(p(1),{'Predict Classes in'; 'each Kfold (PCA)'});xlabel(p(1),'Sample');p(1).YTick=1:nclass;
...
til=legend(p(3),Kf,'Location','best');ylabel(p(3),{'Predict Classes in'; 'each Kfold (PCA)'});
